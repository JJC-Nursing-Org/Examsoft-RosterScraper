...
from main_proj.backend.rs_datamodel import StudentRow
from main_proj.backend.rs_extractors import *
    ...
    def _parse_html(self) -> (List[StudentRow], List[Dict[str, str]]):
        ...
        for idx, rc in enumerate(row_candidates, start=1):
            ...
            for k in cellmap.keys():
                if k not in UtilsRS()._LABEL_MAP.values() and k not in ("lastName","firstName","studentId","email","status","dateInvited","lastLoginDate","lastUserRegistration.registrationDate","assessmentTimeMultiplier","isLabUser","isNonSecure","actions"):
